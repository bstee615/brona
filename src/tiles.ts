...
class TileObject extends GameObject {
    ...
    constructor(map: Tilemap, x: number, y: number, w: number, h: number, tilerow: number, tilecol: number, name: string) {
        super(x, y, w, h);
...
export class Tilemap {
    ...
    loadSprite(row: number, col: number): Sprite {
        ...
        return new Sprite(this.filepath, 
        col * this.tilewidth, row * this.tilewidth,
    ...
    loadTile(tilerow: number, tilecol: number, name = "Unnamed", x = 0, y = 0, w = 1, h = 1): TileObject {
        ...
        const tile = new TileObject(this,
            x, y, w, h, 
            tilerow,
            tilecol,
            name);
